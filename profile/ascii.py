...
DEFAULT_SRC_CANDIDATES = [
    os.path.join(HERE, "source-prepped.png"),
    os.path.join(HERE, "..", "source-prepped.png"),
    os.path.join(HERE, "pic.jpg"),
]
SRC = sys.argv[1] if len(sys.argv) > 1 else next(
    (path for path in DEFAULT_SRC_CANDIDATES if os.path.exists(path)),
    DEFAULT_SRC_CANDIDATES[0],
)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">gourav@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'gourav@github:~$ whoami <tspan fill="{INK}"><br/>Gourav Suresh</tspan></text>')
